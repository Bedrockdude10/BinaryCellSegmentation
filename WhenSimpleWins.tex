...
Nucleus segmentation in histopathology images underlies a broad range of clinical and research workflows, from tumor microenvironment quantification to cell-cycle analysis. Computational pathology increasingly scales transformer-based foundation models to hundreds of millions of parameters pretrained on millions of whole-slide images~\cite{chen2024uni,vorontsov2024virchow}.

This eases the labeled-data shortage but constrains deployment in ways recent work has documented: foundation models are typically restricted to linear probing rather than full fine-tuning due to memory and stability constraints~\cite{tizhoosh2025why,campanella2025clinical}; their robustness to cross-site distribution shift has been questioned~\cite{dejong2025current}; and their H\&E-pretrained features transfer poorly to non-standard stains such as IHC~\cite{hua2024pathoduet}. For deployment scenarios such as edge-deployed microscopy in under-served settings~\cite{salido2025realtime} and veterinary or rare-disease research~\cite{xiao2025review}, where annotation budgets permit only tens of whole-slide images, lightweight from-scratch architectures remain the practical choice.

This raises a concrete design question: within the space of lightweight, from-scratch architectures, which encoder is best in the low-data regime? We answer it with a controlled ablation comparing VGG, ResNet, and Swin encoders on PanNuke and the genuinely small-data MoNuSeg (37 images), holding the decoder and encoder interface fixed so the backbone is the only variable. A VGG encoder matches ResNet and outperforms pretrained Swin on PanNuke, and reaches MedT-level Dice (MedT being a transformer built for small medical datasets) while outperforming pretrained Swin on MoNuSeg.
...
\noindent\textbf{Contributions.} (1)~A controlled encoder-family ablation within a shared U-Net framework on PanNuke and MoNuSeg; (2)~evidence that a from-scratch VGG encoder reaches MedT-level performance (MedT being a transformer purpose-built for small medical datasets) on MoNuSeg while beating pretrained Swin on both; (3)~inference-cost measurements showing VGG is Pareto-dominant over Swin-T on accuracy and inference cost.
...
\noindent\textbf{Testbed.} All results run on a single NVIDIA Tesla~V100-SXM2-32GB in
fp32: accuracy averaged over 5 training seeds, inference cost
(Table~\ref{tab:inference_cost}) measured separately on an idle GPU. Training uses batch
size 8 and the recipe of Section~\ref{sec:training_details}, with data pipeline,
augmentation, optimizer, and decoder fixed so that only the encoder varies. Metrics are Dice and IoU on the held-out splits (PanNuke fold3; MoNuSeg 14-image test set).
...
Pretraining moves it by 0.001~Dice (0.825 from scratch against 0.824
pretrained), so ImageNet initialisation neither helps nor hurts here and the gap
to VGG is not a matter of those weights. This 0.030 Dice gap (paired, 95\% CI $[0.029, 0.031]$) is an order of magnitude larger than the BCE-vs-Dice variation
...
VGG & Dice & \textbf{0.856}\,(0.001) & \textbf{0.748}\,(0.001) & 84.8\,(17.5) & 34.0M \\
...
not comparable to it on equal terms, and outperforms pretrained Swin
(0.758~$\pm$~0.007) by 0.036~Dice, 95\% CI $[0.021, 0.052]$. The MedT comparison is not strictly controlled (different training-set size and input protocol; see Limitations), so we read it only as VGG \emph{reaching} MedT-level Dice. That caveat is confined to this cross-paper anchor: VGG, ResNet, and Swin share the identical 37-image set and protocol, so our controlled VGG-over-Swin result is unaffected;
the gap is comparable at both data scales.

\begin{table}[t]
\centering
\caption{Test results on MoNuSeg (14 test images). Dice computed per image, averaged over the test set. MedT results are published figures (30 training images, $512\times512$ input); ours use 37 training images, $256\times256$ patches, reported as the mean over 5 training
seeds with the standard deviation in parentheses ($n{=}5$). Best result in \textbf{bold}.}
...
VGG (ours, 37 imgs) & \textbf{0.794}\,(0.011) & \textbf{0.660}\,(0.015) & 34.0M \\
\bottomrule
\end{tabular}
\end{table}
...
The VGG--ResNet result of Section~\ref{sec:pannuke_results} is unsurprising:
...
are likewise near-identical (0.854 vs.\ 0.856 Dice; 0.746 vs.\ 0.748 IoU), both gaps
below the 0.005~Dice we treat as material. With moderate foreground
...
Table~\ref{tab:inference_cost} reports inference cost on our testbed (median forward-pass latency over 300 iterations on a single $256\times256$ input after 50 warmup iterations; peak memory via \texttt{torch.cuda.max\_memory\_allocated}), answering Q5. These support the lightweight-deployment framing rather than serving as optimized benchmarks (eager-mode PyTorch, no fusion or quantization).

\begin{table}[t]
\centering
\caption{Single-image inference cost at $256\times256$, batch size~1, fp32, on one
idle NVIDIA Tesla~V100-SXM2-32GB. Latency is the median of 300 synchronised forward
passes after 50 warmup iterations; peak memory covers weights and activations. VGG is
the fastest and most memory-efficient learned model.}
\label{tab:inference_cost}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Model} & \textbf{Params} & \textbf{Latency (ms)} & \textbf{Peak Memory (MB)} \\
\midrule
VGG U-Net & 34.0M & 7.1 & 264.9 \\
ResNet U-Net & 37.5M & 9.6 & 277.2 \\
Swin-T U-Net (pretrained) & 86.8M & 18.4 & 618.8 \\
\bottomrule
\end{tabular}
\end{table}

Swin uses roughly $2.3\times$ the memory and $2.6\times$ the latency of VGG for no accuracy gain.
...
\noindent\textbf{Implications for resource-constrained deployment.} Every configuration we train fits a single 8GB consumer GPU: peak training memory was 3.6~GiB for VGG and 5.4~GiB for pretrained Swin-T, the heaviest we ran. A VGG U-Net (${\sim}34$M parameters) trains overnight with no external data, directly actionable for the edge-microscopy, veterinary, and non-H\&E settings~\cite{xiao2025review,neal2025artificial} that motivate this work.

\noindent\textbf{The role of pretraining.} ImageNet pretraining leaves Swin's
accuracy essentially unchanged (0.824 against 0.825 from scratch), so the
0.030~Dice gap to VGG is not attributable to those weights. We believe the
skip-connection resolution bottleneck of Section~\ref{sec:intuition} is
responsible but have not isolated it: a second Swin variant, a resolution-matched
decoder, or the differing schedule could each contribute. Whether H\&E-matched
pretraining (e.g., UNI~\cite{chen2024uni}) closes the gap would discriminate
between them.

\subsection{Limitations}

Several limitations bound these conclusions. Tables~\ref{tab:pannuke_results} and~\ref{tab:monuseg_results} report mean and standard deviation over five training seeds on one fixed split, and every difference we claim has a paired 95\% confidence interval excluding zero; this bounds training-seed variance, not split variance, which with 37 MoNuSeg images is plausibly larger. We give intervals rather than non-parametric tests because at $n{=}5$ a two-sided signed-rank $p$ cannot fall below 0.0625. We did not sweep learning rates, so cannot quantify the comparison's sensitivity to them; from-scratch Swin's instability at $10^{-2}$ suggests it is not negligible. Pretrained Swin reached its 100-epoch cap in two of five seeds, so its schedule may bound it, though those two seeds scored no lower than the three that stopped on patience. We compare only three encoders, and at ${\sim}34$M parameters VGG is not compact: we cannot place it among genuinely compact encoders such as MobileNet-, EfficientNet- or ConvNeXt-Tiny-based U-Nets. We study only binary segmentation, not instance separation of touching nuclei; the MedT comparison is uncontrolled in training-set size (37 vs.\ 30) and protocol ($256\times256$ tiles vs.\ one $512\times512$ pass); and all data are H\&E, leaving the non-H\&E stains (IHC, fluorescence) that most motivate this work untested.

%% ============================================================================
\section{Conclusion}
%% ============================================================================

We compared CNN and transformer encoders for nucleus segmentation under real-world resource constraints. In a shared U-Net on PanNuke and MoNuSeg, a from-scratch VGG encoder (34M parameters) outperforms pretrained Swin on both and reaches published MedT-level performance on MoNuSeg (0.794 vs.\ 0.796 Dice, a rough equivalence given protocol differences)---a margin consistent across both data scales. For pipelines with tens of annotated images and single-GPU infrastructure, a VGG U-Net encoder is thus the strongest of the three encoders we compare; transformers may need far more data than these benchmarks provide to match it.

\noindent\textbf{Future work.} Promising directions: a resolution-matched Swin, or a CNN with equally reduced skip resolution, to separate tokenization from encoder family; pretraining matched to H\&E rather than ImageNet; the data scale at which pretrained Swin matches VGG; hybrid CNN-transformer encoders; and memory-matched comparison with foundation models (UNI, Virchow2).
